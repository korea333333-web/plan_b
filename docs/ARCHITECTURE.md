...
자동 생성 입력값을 바꾸면 React의 기존 미리보기 배열을 비워, 이전 조건으로 만든 결과를 그대로 저장하지 않게 한다.
...
이번 주 매화는 현재 주에 펼친 발생 회차 중 `completed`인 회차 수를 사용하며, 완료 회차 1개를 꽃 1송이로 렌더링한다. 사용자 지정 주간·월간 기간 필터는 없다.
